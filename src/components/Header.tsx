...
const Header = () => {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 border-b border-outline-variant/20 ${isScrolled
          ...
          <div className="hidden lg:flex items-center gap-10">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className={`font-technical text-[9px] font-bold tracking-[0.2em] transition-all duration-300 hover:text-tertiary relative group/link ${pathname === link.href ? 'text-tertiary' : 'text-on-surface-variant/70'
                  }`}
              >
                {link.name}
      ...
      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 z-[60] bg-background/95 backdrop-blur-xl transition-all duration-500 lg:hidden ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
        }`}>
          ...
          <div className="space-y-12">
            {navLinks.map((link, i) => (
              <Link
                ...
                href={link.href}
                onClick={() => setIsMobileMenuOpen(false)}
                className={`block text-4xl font-display font-bold tracking-tighter transition-all duration-300 ${pathname === link.href ? 'text-tertiary translate-x-4' : 'text-on-surface-variant hover:text-on-surface hover:translate-x-4'
                  }`}
